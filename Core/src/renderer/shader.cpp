...
namespace core {
    ...
    Shader::Shader(const std::string& nVertexShader, const std::string& nFragmentShader)
    {
    ...
    std::shared_ptr<Shader> ShaderManager::GetShader(const char* shaderName)
    {
        return m_shaders.find(shaderName) == m_shaders.end() ? nullptr : m_shaders[shaderName];
    }

    void ShaderManager::AddShader(const char* shaderName, std::shared_ptr<Shader> shaderPtr)
    {
        m_shaders[shaderName] = shaderPtr;
    }
}